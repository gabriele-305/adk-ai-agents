...
def count_characters(text: str, tool_context: ToolContext) -> Dict[str, Any]:
    ...
    if char_count < MIN_LENGTH:
        ...
        return {
            ...
            "char_needed": chars_needed,
            "message": f"Post is too short. Add {chars_needed} more characters to reach the minimum length of {MIN_LENGTH}."
        }
    elif char_count > MAX_LENGTH:
        char_to_remove = char_count - MAX_LENGTH
        tool_context.state["review_status"] = "fail"
        return {
            "result": "fail",
            "char_count": char_count,
            "char_to_remove": char_to_remove,
            "message": f"Post is too long. Remove {char_to_remove} characters to meet the maximum length of {MAX_LENGTH}"
        }
    else:
        return {
            "result": "pass",
