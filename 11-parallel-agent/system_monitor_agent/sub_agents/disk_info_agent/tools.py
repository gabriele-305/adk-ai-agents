...
def get_disk_info() -> Dict[str, Any]:
    ...
    try:
        ...
        for partition in psutil.disk_partitions():
            try:
                partitino_usage = psutil.disk_usage(partition.mountpoint)

                if partitino_usage.percent > 85:
                    partitions_over_threshold.append(
                        f"{partition.mountpoint} ({partitino_usage.percent:.1f}%)"
                    )

                total_space += partitino_usage.total
                used_space += partitino_usage.used
                ...
                disk_info["partitions"].append(
                    {
                        ...
                        "total_size": f"{partitino_usage.total / (1024 ** 3):.2f} GB",
                        "used": f"{partitino_usage.used / (1024 ** 3):.2f} GB",
                        "free": f"{partitino_usage.free / (1024 ** 3):.2f} GB",
                        "percentage": f"{partitino_usage.percent:.1f}%",
                    }
                )
