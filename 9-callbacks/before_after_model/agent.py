...
def after_model_callback(callback_context: CallbackContext, llm_reponse: LlmResponse) -> Optional[LlmResponse]:
    """
    ...
    if not llm_reponse or not llm_reponse.content or not llm_reponse.content.parts:
        return None
    
    response_text = ""
    for part in llm_reponse.content_parts:
    ...
    if modified:
        ...
        modified_parts = [copy.deepcopy(part) for part in llm_reponse.content.parts]

        for i, part in enumerate(modified_parts):
            if hasattr(part, "text") and part.text:
                modified_parts[i].text = part.text
...
root_agent = LlmAgent(
    ...
    description="An agent that demonstrate callbacks for content filtering and logging",
    instruction="""
